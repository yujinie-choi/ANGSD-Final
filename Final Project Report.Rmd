...
Looking at our top 10 genes of significant value for `young vs hgps` and `old vs hgps` and comparing against the top 10 genes that had significant values for `young vs hgps` while being filtered for non-significant value `old vs hgps` genes on a Volcano Plot. It looks like these filtered genes do not have very high log-fold change, unfortunately. 

```{r volcano-plot, warning=FALSE, message=FALSE, fig.height=8}
...
```{r aging-geneset, warning=FALSE, message=FALSE, fig.height=10, fig.show='hide'}
...
![**Fig. 5. GSEA results for DEMAGALHAES_AGING_UP and DEMAGALHAES_AGING_DN:** The GSEA plots would not stack, so I'm embedding an image directly, but you can run the code block to see the same results!](./Images/gsea_results.png)
